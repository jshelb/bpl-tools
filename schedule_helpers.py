...
def teams_from_signup(file_path: str):
    """
    get list of teams from the signup sheet. Column header must be "Team Names" on the sheet

    Args:
        file_path (str): name of signup file in same folder
    ...
    # Convert the values from the column to a list of strings
    team_names_list = df['Team Names'].tolist()
...
def generate_schedule(teams: list[str], num_wks: int, num_groups: int):
    ...
    for _ in tqdm(range(iterations), desc="Optimizing Schedule", unit="iteration"):
...
def print_summary(summary):
    ...
    print(f"Most Played Matchup: {summary['Most Played Matchup']} (Played {summary['Times Played']} times)")
    # print("All Matchups:")
    # for matchup, count in summary['All Matchups']:
